...
UserProfileSchema.methods.generateHash = function (rawPass, callback) {
    scrypt.kdf(rawPass, { N: 1, r: 1, p: 1 }, function (err, result) {
        if (err) {
...
UserProfileSchema.methods.isValidPassword = function (password, callback) {
    var userSchema = this;
    return scrypt.verifyKdf(userSchema.password, new Buffer(password), function onResult(err, result) {
        if (err) {
...
UserProfileSchema.pre('save', function onBeforeSave(next) {
    var userSchema = this;
    if (!this.isModified('password')) {
        return next();
    } else {
        this.generateHash(userSchema.password, function onHashResult(err, result) {
            if (err) {
